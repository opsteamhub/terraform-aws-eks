...
The first successful `Release` workflow run creates `v1.0.0` at the audited,
transitively pinned legacy commit recorded in `.release-baselines.json`. It never
points that tag at the release-automation commit or at the breaking v2
implementation. The workflow refuses to move `v1.0.0` if the tag already exists
at another commit.

`bootstrap-sha` in `release-please-config.json` is intentionally separate: it is
the last pre-v2 commit on the default-branch history from which Release Please
starts reading commits. The legacy release SHA does not need to be an ancestor of
the default branch; the workflow fetches and verifies it before creating the tag.
