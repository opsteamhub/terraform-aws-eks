...
| --- | --- |
| Terraform `~> 1.5.1` rejected current Terraform; AWS provider was pinned to an old 5.x patch | Terraform 1.7+ and AWS provider 6.x ranges, tested with current schemas |
...
- Kubernetes resources, Helm releases, GitOps bootstrap, Cluster Autoscaler, or Karpenter installation.
...
- EKS Auto Mode, Fargate profiles, hybrid nodes, Outposts, and EKS Anywhere.
- Windows, Bottlerocket, GPU, or custom AMI bootstrap automation. These can use explicit `ami_type` or `image_id`/`user_data`, but require separate tested examples before becoming supported presets.
...
1. Run the v2 graph through a disposable AWS sandbox, including private endpoint access, add-on readiness, node join, Access API, IRSA, Pod Identity, and destroy behavior.
2. Publish a versioned v2 release only after the dependency-free EKS PR is approved and the sandbox evidence is attached.
3. Add dedicated presets only when there is a real consumer and acceptance test for Auto Mode, Bottlerocket/ARM/GPU, Fargate, or Karpenter.
